#pragma once
/*----------------------------------------------------------------------------/
/ TJpgDec - Tiny JPEG Decompressor R0.03 - header-only C++ port
/-----------------------------------------------------------------------------/
...
/
/-----------------------------------------------------------------------------/
/ This file merges ChaN's original "tjpgd.h" (public API/struct declarations)
/ and "tjpgd.c" (implementation) into a single header so the decoder can be
/ dropped into a project with a plain #include - no separate compilation
/ unit to add to a build system. Every algorithm, table and comment below is
/ otherwise unchanged from R0.03; the only mechanical differences from the
/ original two-file split are:
/  - every function that used to live in tjpgd.c is now `inline` (not
/    `static`) so the linker merges the one definition across every
/    translation unit that includes this header, instead of either
/    duplicating it (a plain `static` in a header) or erroring with multiple
/    definitions (no specifier at all);
/  - the small set of helper functions that were file-local to tjpgd.c
/    (alloc_pool, create_qt_tbl, create_huffman_tbl, huffext, bitext,
/    restart, block_idct, mcu_load, mcu_output, BYTECLIP) now live in the
/    tjpgd_detail namespace instead of at file (translation-unit) scope, so
/    they can no longer collide with same-named symbols in an unrelated
/    header some other library happens to pull into the same translation
/    unit. JDEC/JRECT/JRESULT and the jd_prepare()/jd_decomp() entry points
/    are deliberately left at global scope, unqualified, exactly as R0.03
/    ships them - that pairing is TJpgDec's well-known public API, reused
/    verbatim by many other ports, and changing it here would break the
/    "drop-in" nature of this conversion for no benefit;
/  - the pre-C99-stdint.h `#if defined(_WIN32)` fallback typedefs are gone in
/    favor of unconditionally including <cstdint> - this is a C++17 library
/    (see the top-level CMakeLists.txt), so a standard <cstdint> is always
/    available;
/  - the lookup tables (Zig, Ipsf, and JD_TBLCLIP's Clip8) are `inline
/    constexpr` (a C++17 feature - see the point above) rather than `static
/    const`, again so there is exactly one copy in the final binary rather
/    than one per translation unit.
/ See tjpgd_config.h for the JD_* configuration macros (JD_SZBUF, JD_FORMAT,
/ JD_USE_SCALE, JD_TBLCLIP, JD_FASTDECODE) - unchanged from the original
/ tjpgdcnf.h other than being individually #ifndef-guarded so a consumer can
/ override them with a `-D` instead of editing this copy of the file.
/-----------------------------------------------------------------------------/
...
inline constexpr uint16_t Ipsf[64] = {	/* See also aa_idct.png */
...
#define BYTECLIP(v) tjpgd_detail::Clip8[(unsigned int)(v) & 0x3FF]

inline constexpr uint8_t Clip8[1024] = {
	/* 0..255 */
...
inline void* alloc_pool (	/* Pointer to allocated memory block (NULL:no memory available) */
...
inline JRESULT create_qt_tbl (	/* 0:OK, !0:Failed */
	JDEC* jd,				/* Pointer to the decompressor object */
...
inline JRESULT create_huffman_tbl (	/* 0:OK, !0:Failed */
	JDEC* jd,					/* Pointer to the decompressor object */
...
inline int huffext (	/* >=0: decoded data, <0: error code */
	JDEC* jd,			/* Pointer to the decompressor object */
...
inline int bitext (	/* >=0: extracted data, <0: error code */
	JDEC* jd,			/* Pointer to the decompressor object */
...
)
{
	...
	if (!JD_USE_SCALE || jd->scale != 3) {	/* Not for 1/8 scaling */
		...
		if (JD_USE_SCALE && jd->scale) {
			unsigned int x2, y2, r, g, b, s, w, a;
			uint8_t *op;
			...
			for (iy = 0; iy < my; iy += w) {
				for (ix = 0; ix < mx; ix += w) {
					...
					r = g = b = 0;
					for (y2 = 0; y2 < w; y2++) {	/* Accumulate RGB value in the square */
						for (x2 = 0; x2 < w; x2++) {
	...
	if (rx < mx) {	/* Is the MCU spans rigit edge? */
		...
		for (y2 = 0; y2 < ry; y2++) {
			for (x2 = 0; x2 < rx; x2++) {	/* Copy effective pixels */
				*d++ = *s++;
				if (JD_FORMAT != 2) {
					*d++ = *s++;
					*d++ = *s++;
...
)
{
  ...
  jd->swap = tmp; // Restore the swap flag
	...
	do {
		...
		ofs++;
		marker = (uint16_t)(marker << 8 | seg[0]);
	} while (marker != 0xFFD8);
	...
	for (;;) {				/* Parse JPEG segments */
		...
		ofs += 4 + (unsigned int)len;		/* Number of bytes loaded */
